...
uint8_t vs_uart_buf[vs_rx_len*2];
...
static void Vs_Rx_Callback(void)   //接收到数据后的中断回调处理
{
    ...
    #if(Tongji == 1) 
	{
        memcpy(vs_uart_buf, vs_usart_instance->recv_buff, sizeof(vs_receive_packet_t)); 

        VS_UnPack_Data_ROS2(vs_uart_buf, &vs_aim_packet_from_nuc, sizeof(vs_receive_packet_t));
	}
	#elif(TianJin == 1)
	{
        memcpy(vs_uart_buf, vs_usart_instance->recv_buff, sizeof(output_bytes_t));
        NV_UnPack_Data_ROS2(vs_uart_buf, &nx_auto_datas, sizeof(output_bytes_t));
	}
	#endif
} 
