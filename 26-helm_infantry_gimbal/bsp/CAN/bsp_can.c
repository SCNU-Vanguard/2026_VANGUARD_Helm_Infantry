...
CAN_instance_t *can_instances[CAN_MX_REGISTER_CNT] = {NULL};
...
static void CAN_Add_Filter(CAN_instance_t *_instance)
{
	...
	// ---------------------------------- 扩展帧特殊处理 -----------------------------------
	if(_instance->rx_id  > 0x7FF) // 如果接收ID大于0x7FF,说明使用了扩展ID
	{
		/*
		 * 当ExtFiltersNbr为0时,硬件没有扩展过滤器空间。
		 * 这种情况下由全局过滤器放行扩展帧到FIFO0,再由软件按Identifier分发。
		 */
		if (_instance->can_handle->Init.ExtFiltersNbr == 0)
		{
			return;
		}
		fdcan_filter_conf.IdType = FDCAN_EXTENDED_ID; // 扩展ID
	}
	...
	_instance -> fdcan_filter = &fdcan_filter_conf;
...
static void CAN_Service_Init( void )
{
	...
	HAL_FDCAN_ConfigGlobalFilter(&hfdcan1, FDCAN_REJECT, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE);// 标准帧按过滤器接收,扩展帧兜底进FIFO0
	...
	HAL_FDCAN_ConfigGlobalFilter(&hfdcan2, FDCAN_REJECT, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE);
	...
	HAL_FDCAN_ConfigGlobalFilter(&hfdcan3, FDCAN_REJECT, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE);
...
#ifdef FDCAN
static uint8_t FDCAN_DLC_To_Bytes(uint32_t dlc_raw)
{
	/*
	 * 兼容两种DataLength编码:
	 * 1) HAL宏常量: 0x000x0000
	 * 2) 直接DLC码: 0~15
	 */
	uint8_t dlc_code = (dlc_raw <= 0x0FU) ? (uint8_t)dlc_raw : (uint8_t)((dlc_raw >> 16) & 0x0F);
	if (dlc_code <= 8)
	{
		return dlc_code;
	}

	/* 本工程rx_buff仅8字节,超过按8字节截断 */
	return 8;
}
...
static void CANFIFOxCallback(FDCAN_HandleTypeDef *_hfdcan, uint32_t fifox)
{
	...
	while (HAL_FDCAN_GetRxFifoFillLevel(_hfdcan, fifox)) // FIFO不为空,有可能在其他中断时有多帧数据进入
	{
		...
		if (HAL_FDCAN_GetRxMessage(_hfdcan, fifox, &rx_config_header, can_rx_buff) != HAL_OK) // 从FIFO中获取数据
		{
			continue;
		}
		rx_len = FDCAN_DLC_To_Bytes(rx_config_header.DataLength);
		for (size_t i = 0 ; i < idx ; ++i)
		{ // 两者相等说明这是要找的实例
			uint8_t id_type_match = ((rx_config_header.IdType == FDCAN_STANDARD_ID && can_instances[i]->rx_id <= 0x7FFU)
									|| (rx_config_header.IdType == FDCAN_EXTENDED_ID && can_instances[i]->rx_id > 0x7FFU));
			if (_hfdcan == can_instances[i]->can_handle && id_type_match && rx_config_header.Identifier == can_instances[i]->rx_id)
			{
				if (can_instances[i]->can_module_callback != NULL) // 回调函数不为空就调用
				{
					can_instances[i]->rx_len = rx_len;                                         // 将DLC字段转换为实际字节长度
